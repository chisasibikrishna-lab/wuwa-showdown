  ...
  return (
    <div className="bg-[#111318] border border-white/[0.06] rounded-2xl p-6 sm:p-8 shadow-xl shadow-black/20">
      <h2 className="text-white font-semibold text-sm tracking-wide uppercase flex items-center gap-2.5 mb-6">
        <MapPin size={16} className="text-primary" /> Create New Geoguess Operation
      </h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div className="flex flex-col gap-3">
          ...
          {geoImage ? (
            ...
            <button onClick={() => fileInputRef.current?.click()} className="w-full aspect-video border-2 border-dashed border-white/[0.06] bg-white/[0.01] rounded-xl flex flex-col items-center justify-center gap-3 text-white/20 hover:border-primary/30 hover:text-primary/70 hover:bg-primary/[0.02] transition-all duration-200">
              <Upload size={36} />
        ...
        <div className="flex flex-col gap-3">
          <label className="text-white/40 font-medium text-xs tracking-wide uppercase flex justify-between">
            ...
            {geoCoords && <span className="text-primary font-mono bg-primary/5 px-2 py-0.5 rounded-lg border border-primary/15 text-[11px]">{Math.round(geoCoords[0])}, {Math.round(geoCoords[1])}</span>}
          </label>
        ...
        <div className="col-span-1 md:col-span-2 flex flex-col md:flex-row items-end gap-5 bg-white/[0.02] p-6 rounded-xl border border-white/[0.04] mt-2">
          <div className="flex-1 w-full relative">
            ...
            <div className="relative w-full flex items-center">
              <input
                ...
                className="w-full h-14 bg-[#111318] border border-white/[0.08] rounded-xl pl-4 pr-24 text-white font-mono text-lg outline-none focus:border-primary/30 transition-all duration-200 [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none [-moz-appearance:textfield]"
              />
          ...
          <button
            ...
            className="w-full md:w-auto h-14 flex items-center justify-center bg-primary hover:bg-primary-light text-black px-10 rounded-xl font-semibold tracking-wide text-sm transition-all duration-200 disabled:opacity-30 disabled:cursor-not-allowed shadow-primary-md disabled:shadow-none min-w-[220px]"
          >
            Register Mission
          </button>
        </div>
